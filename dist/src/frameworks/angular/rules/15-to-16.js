...
export default {
    ...
    async runScanner() {
        ...
        if (httpHits.length) {
            deprecatedItems.push(...httpHits.map(f => `HttpModule detected (use HttpClientModule) → ${f}`));
        ...
        if (tsTargets.length) {
            deprecatedItems.push(...tsTargets.map(f => `tsconfig target "es5" — prefer "es2015" or later for Angular 16 → ${f}`));
        ...
        if (rxjsCompatHits.length) {
            deprecatedItems.push(...rxjsCompatHits.map(f => `rxjs-compat usage detected (RxJS 7+ recommended) → ${f}`));
        ...
        if (moduleWithProvidersHits.length) {
            deprecatedItems.push(...moduleWithProvidersHits.map(f => `ModuleWithProviders without generic type detected → ${f}`));
        ...
        if (hasAppModule && hasMainTs) {
            deprecatedItems.push("ℹ Angular 16: Standalone bootstrap is recommended but optional. Consider migrating from NgModule-based bootstrap to standalone.");
        }
        /**
        ...
        if (zoneJsHits.length) {
            deprecatedItems.push(...zoneJsHits.map(f => `zone.js detected → Angular 16 supports zoneless apps (optional). Review if you want to migrate → ${f}`));
