...
export default {
    async runScanner() {
        ...
        if (entryHits.length) {
            deprecatedItems.push(...entryHits.map(f => `entryComponents still present → ${f}`));
        ...
        if (httpHits.length) {
            deprecatedItems.push(...httpHits.map(f => `HttpModule detected (must use HttpClientModule) → ${f}`));
        ...
        if (compatHits.length) {
            deprecatedItems.push(...compatHits.map(f => `rxjs-compat still used → ${f}`));
        ...
        if (mwpHits.length) {
            deprecatedItems.push(...mwpHits.map(f => `ModuleWithProviders without generic type → ${f}`));
        ...
        if (animHits.length) {
            deprecatedItems.push(...animHits.map(f => `AnimationModule.forRoot detected → use provideAnimations() instead → ${f}`));
        ...
        if (tsTargets.length) {
            deprecatedItems.push(...tsTargets.map(f => `tsconfig target "es5" — update recommended → ${f}`));
        ...
        if (hasAppModule) {
            deprecatedItems.push("ℹ Angular 18 recommends standalone bootstrap (NgModule bootstrap is still supported but discouraged).");
        }
        const httpClientProvideHits = find(/provideHttpClient/g);
        if (!httpClientProvideHits.length) {
            deprecatedItems.push("ℹ Angular 18 recommends provideHttpClient() for bootstrap (optional).");
        }
        /**
        ...
        if (zoneHits.length) {
            deprecatedItems.push(...zoneHits.map(f => `zone.js detected → Angular supports zoneless mode (optional) → ${f}`));
        }
