...
export default {
    async runScanner() {
        ...
        if (tsVersionInfo && tsVersionInfo.requiresUpgrade) {
            deprecatedItems.push(`TypeScript version appears to be "${tsVersionInfo.raw}". Angular 19 expects TypeScript 5.6+.`);
        ...
        if (fs.existsSync(angularJsonPath)) {
            ...
            if (content.includes("@angular-devkit/build-angular:protractor")) {
                deprecatedItems.push(`Protractor builder detected in angular.json (removed in Angular 19). Migrate e2e tests to a supported runner.`);
        ...
        if (legacyTargets.length) {
            deprecatedItems.push(...legacyTargets.map(f => `tsconfig target "es5" detected → modernize recommended → ${f}`));
            autoFixTargets.push("tsTargetModernize");
        }
        return { deprecatedItems, autoFixTargets };
        // ---------- helpers ----------
        function detectTypescriptVersion(projectRoot) {
            const pkgPath = path.join(projectRoot, "package.json");
            if (!fs.existsSync(pkgPath))
                return null;
            try {
                const pkg = fs.readJSONSync(pkgPath);
                const raw = (pkg.devDependencies && pkg.devDependencies.typescript) ||
                    (pkg.dependencies && pkg.dependencies.typescript);
                if (!raw || typeof raw !== "string")
                    return null;
                const match = raw.match(/(\d+)\.(\d+)\.(\d+)/);
                if (!match)
                    return { raw, major: 0, minor: 0, patch: 0, requiresUpgrade: true };
                const major = Number(match[1]);
                const minor = Number(match[2]);
                const patch = Number(match[3]);
                // Require >= 5.6.x
                const requiresUpgrade = major < 5 || (major === 5 && minor < 6);
