...
export default {
    ...
    async runScanner() {
        ...
        if (httpHits.length) {
            deprecatedItems.push(...httpHits.map(f => `HttpModule still used (blocking future upgrades) → ${f}`));
        ...
        if (bamHits.length) {
            deprecatedItems.push(...bamHits.map(f => `BrowserAnimationsModule detected — consider provideAnimations() → ${f}`));
        ...
        if (hasAppModule && hasMain) {
            deprecatedItems.push("ℹ Angular 17: standalone bootstrap (no NgModule) is recommended — optional migration.");
        }
        /**
        ...
        if (zoneHits.length) {
            deprecatedItems.push(...zoneHits.map(f => `zone.js detected — Angular 17 supports "zoneless" mode. Optional migration → ${f}`));
        }
