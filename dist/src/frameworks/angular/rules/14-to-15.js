...
export default {
    ...
    async runScanner() {
        ...
        if (tsTargets.length) {
            deprecatedItems.push(...tsTargets.map(f => `tsconfig target "es5" — prefer "es2015" or later → ${f}`));
        ...
        if (animHits.length) {
            deprecatedItems.push(...animHits.map(f => `AnimationModule.forRoot is not supported → use provideAnimations() instead → ${f}`));
        ...
        if (bamHits.length) {
            deprecatedItems.push(...bamHits.map(f => `BrowserAnimationsModule detected — consider \`provideAnimations()\` (optional) → ${f}`));
        ...
        if (routerHits.length) {
            deprecatedItems.push(...routerHits.map(f => `RouterModule.forRoot: review strict options for Angular 15 (optional) → ${f}`));
            autoFixTargets.push("routerStrictOptionsReview");
        }
        /**
         * 7) Optional: upgrade to standalone bootstrap (manual guidance only)
         */
        deprecatedItems.push(`ℹ Angular 15 supports standalone apps — conversion is optional (no breaking change).`);
