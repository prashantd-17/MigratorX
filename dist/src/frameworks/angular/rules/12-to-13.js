...
export default {
    ...
    async runScanner() {
        ...
        if (fs.existsSync(webpackConfig) || fs.existsSync(extraWebpack)) {
            deprecatedItems.push(`⚠ Custom Webpack config detected — verify Webpack 5 compatibility manually`);
        ...
        if (cvaHits.length) {
            deprecatedItems.push(...cvaHits.map(f => `ControlValueAccessor "writeValue?" signature deprecated → ${f}`));
        ...
        if (i18nHits.length) {
            deprecatedItems.push(...i18nHits.map(f => `Legacy i18n message identifier format detected → ${f}`));
        ...
        if (tsConfigHits.length) {
            deprecatedItems.push(...tsConfigHits.map(f => `tsconfig target "es5" → modernize recommended → ${f}`));
