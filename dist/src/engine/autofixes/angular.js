 ...
 */
function getTsConfigPath(rootDir) {
 ...
 */
export async function applyEnableIvyFix(rootDir) {
    const candidates = [
        "tsconfig.json",
        "tsconfig.app.json",
    ...
    for (const file of candidates) {
        ...
        try {
            ...
            if (!/enableIvy\s*:\s*false/.test(content))
                continue;
            const updated = content.replace(/enableIvy\s*:\s*false\s*,?/g, "");
            if (updated !== content) {
                fs.writeFileSync(fullPath, updated, "utf8");
                console.log(chalk.yellow(`🔧 Auto-fix: removed 'enableIvy: false' in ${file}`));
                changedFiles++;
            }
        }
        catch (err) {
 ...
 */
export async function applyHttpModuleFix(rootDir) {
    ...
    for (const sf of sourceFiles) {
        ...
        let httpClientImport = imports.find((imp) => imp.getModuleSpecifierValue() === "@angular/common/http");
        if (!httpClientImport) {
    ...
    if (changedFiles > 0) {
        await project.save();
        console.log(chalk.yellow(`🔧 Auto-fix: HttpModule → HttpClientModule in ${changedFiles} module file(s).`));
    }
    return changedFiles;
}
/**
 ...
 */
export async function applyEntryComponentsFix(rootDir) {
    const tsconfigPath = getTsConfigPath(rootDir);
    if (!tsconfigPath) {
    ...
    if (changedFiles > 0) {
        await project.save();
        console.log(chalk.yellow(`🔧 Auto-fix: removed entryComponents in ${changedFiles} module file(s).`));
    }
    return changedFiles;
}
/**
 ...
 */
export async function applyBrowserslistIEFix(rootDir) {
    let changedFiles = 0;
    // .browserslistrc
    ...
    if (fs.existsSync(browserslistRc)) {
        ...
        const filtered = lines.filter((line) => !/ie\s*11/i.test(line.trim()));
        if (filtered.join("\n") !== content) {
            fs.writeFileSync(browserslistRc, filtered.join("\n"), "utf8");
            console.log(chalk.yellow("🔧 Auto-fix: removed IE11 from .browserslistrc"));
            changedFiles++;
        }
    }
    // browserslist file
    ...
    if (fs.existsSync(browserslistFile)) {
        ...
        if (filtered.join("\n") !== content) {
            ...
            console.log(chalk.yellow("🔧 Auto-fix: removed IE11 from browserslist file"));
            changedFiles++;
        }
    }
    ...
    if (fs.existsSync(packageJsonPath)) {
        try {
            ...
            if (modified) {
                ...
                console.log(chalk.yellow("🔧 Auto-fix: removed IE11 from package.json browserslist"));
                changedFiles++;
            }
        }
        catch {
 ...
 */
export async function applyTsTargetModernize(rootDir) {
    ...
    for (const file of candidates) {
        ...
        try {
            ...
            if (json?.compilerOptions?.target === "es5") {
                ...
                console.log(chalk.yellow(`🔧 Auto-fix: tsconfig target es5 → es2015 in ${file}`));
                changedFiles++;
            }
        }
        catch {
 ...
 */
export async function applyCommonJsReviewNote(_rootDir) {
    console.log(chalk.yellow("ℹ commonJsUsageReview: Detected require() usage. Manual review is recommended; automatic conversion is not implemented yet."));
}
/**
 * 7) AnimationModule.forRoot → provideAnimations()
 */
export async function applyAnimationModuleFix(rootDir) {
    const tsconfigPath = getTsConfigPath(rootDir);
    if (!tsconfigPath)
        return 0;
    const project = new Project({ tsConfigFilePath: tsconfigPath });
    ...
    for (const sf of moduleFiles) {
        ...
        if (changed) {
            const imp = sf.getImportDeclaration(i => i.getModuleSpecifierValue() === "@angular/platform-browser/animations");
            if (!imp) {
                sf.addImportDeclaration({
 ...
 */
export async function applyRxjsCompatRemovalFix(rootDir) {
    let changed = false;
 ...
 */
export async function applyModuleWithProvidersGenericFix(rootDir) {
    const tsconfigPath = getTsConfigPath(rootDir);
    if (!tsconfigPath)
        return 0;
    const project = new Project({ tsConfigFilePath: tsconfigPath });
    ...
    return changedCount;
    function inferModuleName(sf) {
        const cls = sf.getClasses()[0];
        return cls ? cls.getName() + "" : "unknown";
    }
}
export async function applyTypescript56Upgrade(rootDir) {
    const packageJsonPath = path.join(rootDir, "package.json");
    if (!fs.existsSync(packageJsonPath)) {
        return 0;
    }
    let changed = false;
    const pkg = fs.readJSONSync(packageJsonPath);
    function maybeBump(container) {
        if (!container || typeof container.typescript !== "string")
            return;
    ...
    if (changed) {
        fs.writeJSONSync(packageJsonPath, pkg, { spaces: 2 });
        console.log(chalk.yellow("🔧 Auto-fix: updated TypeScript version in package.json to ^5.6.0 for Angular 19 compatibility."));
        return 1;
    }
    return 0;
}
export async function applyProtractorBuilderReview(rootDir) {
    const angularJsonPath = path.join(rootDir, "angular.json");
    if (!fs.existsSync(angularJsonPath))
        return;
    console.log(chalk.yellow("ℹ protractorBuilderReview: Angular 19 removed the Protractor builder. " +
