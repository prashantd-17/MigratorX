...
program.action(async () => {
    ...
    console.log(chalk.greenBright(`🔍 Framework detected → Angular v${currentVersion}`));
    ...
    const { targetVersion } = await inquirer.prompt([
        {
            ...
            validate: input => /^[0-9]+$/.test(input) ? true : 'Enter a valid numeric version'
        }
    ]);
    if (mode === 'logs') {
    ...
    if (mode === 'migrate') {
        ...
        await runMigrateMode(currentVersion, Number(targetVersion), askStep, enableAutofixer);
        return;
    }
});
program.parse();
