  ...
  async runScanner() {
    ...
    if (browserslistFindings.length) {
      deprecatedItems.push(
        ...browserslistFindings.map(
          f => `IE11 is still targeted in browserslist → ${f}`
        )
      );
    ...
    if (ivyHits.length) {
      deprecatedItems.push(
        ...ivyHits.map(f => `ViewEngine / enableIvy:false usage detected → ${f}`)
      );
    ...
    if (tsconfigHits.length) {
      deprecatedItems.push(
        ...tsconfigHits.map(
          f => `TypeScript target "es5" detected (consider modern target) → ${f}`
        )
      );
    ...
    if (commonJsHits.length) {
      deprecatedItems.push(
        ...commonJsHits.map(
          f =>
            `CommonJS-style "require()" detected (may cause Angular CLI warnings) → ${f}`
        )
      );
 ...
 */
function detectIE11Support(projectRoot: string): string[] {
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      if (Array.isArray(browserslist)) {
        const hasIE11 = browserslist.some((entry: string) =>
          /ie\s*11/i.test(entry)
        );
        if (hasIE11) {
        ...
        for (const envKey of Object.keys(browserslist)) {
          ...
          if (Array.isArray(envList)) {
            const hasIE11 = envList.some((entry: string) =>
              /ie\s*11/i.test(entry)
            );
            if (hasIE11) {
 ...
 */
function detectEs5Target(projectRoot: string): string[] {
  const matches: string[] = [];
