  ...
  async runScanner() {
    ...
    if (rxjsCompatHits.length) {
      deprecatedItems.push(
        ...rxjsCompatHits.map(
          f =>
            `rxjs-compat usage detected (RxJS 7 recommended, remove compat) → ${f}`
        )
      );
     ...
     */
    const moduleWithProvidersHits = scanAllFiles(
      /ModuleWithProviders\s*(?!<)/g
    );
    if (moduleWithProvidersHits.length) {
      deprecatedItems.push(
        ...moduleWithProvidersHits.map(
          f =>
            `ModuleWithProviders without generic type detected (consider typing) → ${f}`
        )
      );
    ...
    if (tsTargetHits.length) {
      deprecatedItems.push(
        ...tsTargetHits.map(
          f => `tsconfig target "es5" → modernize recommended → ${f}`
        )
      );
    ...
    if (httpModuleHits.length) {
      deprecatedItems.push(
        ...httpModuleHits.map(
          f => `HttpModule detected (use HttpClientModule instead) → ${f}`
        )
      );
