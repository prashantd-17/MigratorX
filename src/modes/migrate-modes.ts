...
export async function runMigrateMode(
  ...
  askStepConfirmation: boolean,
  enableAutofixer: boolean
) {
    ...
    try {
      ...
      if (runCliUpdate) {
        try {
          ...
          console.error(
            chalk.red(
              `💥 Angular CLI update failed for ${stepLabel}. You may need to fix issues and re-run.`
            )
          );
        ...
        console.log(
          chalk.yellow(
            `⏭ Skipping Angular CLI update for ${stepLabel}. Angular packages might still be on v${v}.`
          )
        );
      }
