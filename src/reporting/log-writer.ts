...
export function generateMarkdownLog({
  ...
  deprecatedItems.forEach(item => {
    const isOptional = item.trim().startsWith("ℹ");
    if (isOptional) optional.push(item);
    ...
    if (match) {
      ...
      if (!fileMap[cleaned]) fileMap[cleaned] = new Set();
      fileMap[cleaned].add(item.split("→")[0].trim());
    }
  });
...
📄 Generated by **MigratorX — Quantum Migration Core**
`;
}
